...
- 结果面板已收敛为 400pt 宽的轻量浮窗：44pt 顶/底工具栏、统一卡片背景、语义色图标按钮与自适应预览/文本高度
...
- 翻译来源支持三种：“自动”“仅系统翻译”“Apple 在线翻译（快捷指令）”
- “自动”优先走「快捷指令」调用 Apple 在线翻译（`ShortcutsTranslationService` 经 `shortcuts run` 调用用户安装的快捷指令），未安装或失败/超时时静默回退系统翻译；`.appleShortcut` 则始终走快捷指令
- Apple 的 `Translation` 框架仅设备端、不向第三方开放在线翻译，故在线能力借道「快捷指令」（跟随系统「设备端模式」开关）实现
- 设置页翻译分区含快捷指令导入步骤引导（iCloud「获取快捷指令」+ 安装状态实时检测）与中↔英语言包管理区块
- 另可选配 `TEXTGRABBER_TRANSLATION_API_URL` 等环境变量接入自建 HTTP 在线 provider（进阶）
- 设置窗口支持修改快捷键
- 设置窗口支持切换识别窗口定位方式
- 设置窗口采用单页分区布局，集中管理截图激活、识别结果、翻译（含快捷指令配置）、语言包与文本编辑设置
- 结果面板在菜单栏模式下优先按图标中心对齐显示
- 截图预览区域与内容卡片等宽，按截图比例自适应高度；长截图在 180pt 视口内无弹性滚动
- 文本识别区域按内容高度在 `8` 到 `24` 行之间伸缩，超出后在区域内部滚动
- 识别完成后会自动将焦点定位到结果文本区域末尾，并使用一级语义色输入光标，方便直接键盘选择、复制和编辑
- 结果面板支持头部 `Pin` 按钮；固定后窗口不会因失焦自动关闭，方便跨应用对照内容
- 生产界面默认展示阅读优化结果，不再提供排版模式切换
- 识别正文和翻译结果使用统一浅色卡片背景；两区正文均为 14pt 一级字色，翻译卡片标题为 `Apple Translate`
- 所有角落工具按钮使用一致的纯图标样式、0.5 秒纯色描边 Tooltip 和 24% hover 背景
- 块编辑默认开启：首次 `⌘A`/`⌃A` 选择当前段落，再次选择全文；设置中可关闭并恢复系统全选行为
- 块选择视觉按完整行片段绘制，复制仍基于真实字符范围；核心范围计算位于 `BlockEditingSelection`
- OCR 后处理规则已从 `OCRService` 拆到 `OCRTextLayoutRules`，集中处理图标噪声、列表标记、段落合并和中英文空格
...
- 当前已重新验证 `swift test`（56 项）和 `./scripts/codex-run.sh --verify`
...
- `Pin`、自动聚焦、编辑后复制、块编辑和截图取消不唤窗已具备测试或真实运行验证
- 当前结果面板与设置页均以真实窗口截图校对；后续重点转向长短文本、深浅色和错误态回归
...
3. 补充测试与回归用例  
   当前已有 56 项单测；块选择视觉、真实快捷键分发和设置页条件布局仍需人工回归。

4. 真机验证翻译在线链路  
   语言包引导与快捷指令导入引导已落地；重点转为真机回归：未装语言包不自动弹下载框、重复翻译不卡住、快捷指令中英双向与「设备端模式」在线效果。
...
3. 对照真实 OCR/翻译结果做视觉回归，重点确认正文、翻译卡片和块选择在长短文本下的表现
4. 为设置条件布局和 AppKit 快捷键分发增加更多自动化覆盖
5. 为真实用户设计翻译资源未就绪时的安装引导
6. 验证在线翻译 provider 的失败回退和超时路径
7. 评估登录启动、Developer ID 签名与 notarization
...
- `Sources/TextGrabberKit/Services/OCRService.swift`  
  负责 Vision OCR、图像增强与候选结果选择。

- `Sources/TextGrabberKit/Services/OCRTextLayoutRules.swift`
  集中管理 OCR 文本清洗、段落合并、列表符号与中英文空格规则。
...
- `Sources/TextGrabberKit/UI/BlockEditingSelection.swift`
  计算块编辑的“当前段落 → 全文”两阶段选择范围。

- `Sources/TextGrabberKit/UI/SettingsView.swift`  
  单页设置界面，包含截图识别、结果、翻译、语言包与文本编辑等分区。
